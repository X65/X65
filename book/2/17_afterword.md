...
| <https://github.com/X65>              | The project GitHub organisation — firmware, schematic, examples, emulator, K816, cc65-dbg, OS/816 |
| <https://x65.zone/emu/>               | Web emulator (WebAssembly build of Emu)                            |
| <https://tracker.x65.zone/>           | SGU Tracker — runs in the browser; desktop builds under `/latest/` |
...
- **`firmware`** — the authoritative implementation of the NORTH, SOUTH, and audio chips on RP2350-family microcontrollers. The behavioural source of truth for this book.
...
At the time this book snapshot was written, the X65 is in the **Gen2 DEV-board** era: two RP2350s split as a NORTH / SOUTH pair, an SGU-1 sound module behind SOUTH, and the Raspberry Pi Radio Module 2 for Wi-Fi and Bluetooth. The SGU-1 sound chip is feature-complete — nine 4-operator FM channels with SID-style filtering, sweeps, and PCM playback, available in Emu and in SGU Tracker. CGIA provides paletted and attribute modes plus HAM6 and affine, at a perfect 60 Hz over DVI-D. OS/816 has a working scheduler, COP-based syscalls, and a small shell; several of its intended subsystems (CIO, signals, bank allocator, system-wide lock registry) are designed and reserved but not yet implemented.
...
- **Music and graphics assets** for shared demos and games — the SGU-1 + SGU Tracker pipeline and the CGIA + image-converter tooling mean a composer or artist can contribute without touching 65C816 assembly.
